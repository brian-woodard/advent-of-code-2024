...
fn main() {
    ...
    println!("Result: {} out of {}", total_antinodes_on_map.len(), total_antinodes);
}
...
fn find_antinodes(antinodes: &mut HashMap<i32, bool>, idx1: i32, idx2: i32, width: i32, height: i32) {
    ...
    let new_x1 = x1 - dist_x;
    let new_y1 = y1 - dist_y;
    let new_x2 = x2 + dist_x;
    let new_y2 = y2 + dist_y;

    if new_x1 >= 0 && new_x1 < width &&
        new_y1 >= 0 && new_y1 < height {
        let idx = (new_y1 * width) + new_x1;
        antinodes.insert(idx, true);
    }

    if new_x2 >= 0 && new_x2 < width &&
