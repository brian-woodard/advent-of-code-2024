...
void CGlRect::Render(const glm::mat4& Projection)
{
   if (!mShader || !mVAO)
   {
      std::cerr << "CGlRect: Shader or VAO is not initialized!" << std::endl;
      return;
   }
   ...
   if (mTranslate.x == 0.0f && mTranslate.y == 0.0f)
   {
   ...
   {
      // Line drawing has vertex starting at mX, mY
      vertices_ll = glm::vec2(0.0f, 0.0f);
      vertices_ur = glm::vec2(mWidth, mHeight);
