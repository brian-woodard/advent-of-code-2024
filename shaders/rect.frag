...
void main()
{
   ...
   vec2 softness_padding = vec2(max(0.0, uEdgeSoftness*2.0 - 1.0),
                                max(0.0, uEdgeSoftness*2.0 - 1.0));
   ...
   {
      dist = RectSDF(pos-uRectSize/2.0, uRectSize/2.0 - uBorderThickness/2.0-1.0, uCornerRadius);
      float blend_amount = smoothstep(-1.0, 1.0, abs(dist) - uBorderThickness / 2.0);
